...
world_faa = world.merge(country_counts, how='left', left_on=shapefile_iso_col, right_on='iso3')
world_faa['aircraft_count'] = world_faa['aircraft_count'].fillna(0)

# Color map
cmap = plt.colormaps['Oranges']
norm = mcolors.Normalize(vmin=world_faa['aircraft_count'].min(), vmax=world_faa['aircraft_count'].max())
world_faa['color'] = world_faa['aircraft_count'].apply(lambda x: '#ffffff' if x==0 else mcolors.to_hex(cmap(norm(x))))

# Plot
fig, ax = plt.subplots(1,1,figsize=(15,8))
world_faa.plot(color=world_faa['color'], ax=ax, edgecolor='black')
sm = cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
fig.colorbar(sm, ax=ax, fraction=0.02, pad=0.05, label='Number of registered aircraft')
ax.set_title('FAA - World Registered Aircraft Population', fontsize=24)
ax.axis('off')
fig.text(0.5, 0.09,
         'Counts include only aircraft registered with the U.S. FAA. Source: May 13, 2025.',
         ha='center', fontsize=10)
plt.show()

print(f"Total aircraft represented on map: {country_counts['aircraft_count'].sum()}")
...
df_aea_clean['country'] = df_aea_clean['country'].replace(manual_country_map)
...
if len(unmatched) > 0:
    print(f"Unmatched AEA countries: {unmatched}")

country_counts_aea = (
    df_aea_clean.dropna(subset=['iso3'])
    .groupby('iso3')
    .size()
...
world_aea['color'] = world_aea['repair_station_count'].apply(lambda x: '#ffffff' if x==0 else mcolors.to_hex(cmap(norm(x))))

fig, ax = plt.subplots(1,1,figsize=(15,8))
world_aea.plot(color=world_aea['color'], ax=ax, edgecolor='black')
sm = cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
fig.colorbar(sm, ax=ax, fraction=0.02, pad=0.05, label='Number of AEA Repair Stations')
ax.set_title('AEA - World Repair Station Distribution', fontsize=24)
ax.axis('off')
fig.text(0.5, 0.09, 'Counts include only repair stations from the AEA 2025-2026 list.', ha='center', fontsize=10)
plt.show()

print(f"Total repair stations represented on map: {country_counts_aea['repair_station_count'].sum()}")
...
df_us = df_faa_clean[df_faa_clean['COUNTRY']=='US']
